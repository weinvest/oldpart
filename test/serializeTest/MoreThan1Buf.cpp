...
BOOST_FIXTURE_TEST_CASE(MoreThan1Buf_Encrypt, MaxBodyLenFixture)
{
    ...
    auto errKey("errKey");
    MoreThan1Buf* deObjErr = serializer.CreateProto<MoreThan1Buf>(pMessage1->GetMessageId());
    BOOST_TEST(false == serializer.Deserialize(*deObj, used4Deserialize, errKey));
    BOOST_TEST(deObjErr->a1 != obj.a1);
    BOOST_TEST(deObjErr->v1 != obj.v1);
    BOOST_TEST(deObjErr->s1 != obj.s1);
}

BOOST_FIXTURE_TEST_CASE(MoreThan1Buf_Compress_And_Encrypt, MaxBodyLenFixture)
{
    MoreThan1Buf obj;
    obj.a1 = 2;
    obj.v1 = 3.2;
    ...
    BOOST_TEST(2 == pMessage1->GetCompressLevel());
    BOOST_TEST(2 == pMessage2->GetCompressLevel());
